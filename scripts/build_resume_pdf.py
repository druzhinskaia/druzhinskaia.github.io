...
def build() -> None:
    ...
    story.append(header)
    story.append(p("Аналитик данных и автоматизации", role))
    story.append(
        p(
            "Excel · Power Query · VBA · 1С · SQL и Python/pandas (базовый уровень)",
            small,
        )
    )
    story.append(
        p(
            "Работаю с финансовыми и операционными данными, проверяю выгрузки из 1С, "
            "автоматизирую регулярную отчетность и контроль качества информации. "
            "Описываю требования, тестирую решения и готовлю материалы для пользователей.",
            base,
        )
    )
    ...
    three_cols = Table(
        [
            [
                ...
                p("<b>Инструменты</b><br/>Excel, Power Query, VBA, 1С:БГУ, SQL и Python/pandas (базовый уровень), Power BI (базовый уровень).", cell),
                p("<b>Формат</b><br/>Удалённая работа. Готова к командировкам.", cell),
            ]
        ],
        colWidths=[59 * mm, 59 * mm, 59 * mm],
    )
    ...
    story.append(bullet("<b>Сен 2026 - н. в. · Старший аналитик по ИИ-автоматизации, МИ ФНС России по управлению долгом.</b> Работаю с данными бухгалтерского и финансового учёта в 1С:БГУ и системах ФНС: сопоставляю платежи и документы, выявляю расхождения и готовлю аналитические материалы. Автоматизирую повторяющиеся проверки в Excel и применяю ИИ-инструменты для отдельных рабочих задач.", base))
    story.append(bullet("<b>Дек 2025 - апр 2026 · Ведущий аналитик, ГАУ ИТЦ \"Соцзащита\" Москвы.</b> Вела проект автоматизации Excel-отчетности для 120-140 подведомственных организаций: анализировала процесс подготовки, обновляла структуру формы, настраивала контроль качества данных и тестировала решение. Подготовила документацию, обучила пользователей и сопровождала переход на новый рабочий инструмент.", base))
    story.append(bullet("<b>Янв 2022 - фев 2026 · Администратор, ИП Дружинская.</b> Координировала операционные процессы: вела реестры и документацию, проверяла качество данных, готовила отчетность и оптимизировала повторяющиеся операции в Excel. Фиксировала договоренности и статусы задач, поддерживала актуальность рабочих таблиц.", base))
    story.append(bullet("<b>Май 2024 - ноя 2024 · Трансферный гид, Fit Holidays.</b> Координация логистики, анализ обратной связи клиентов, выявление повторяющихся проблем и подготовка предложений по улучшению качества сервиса.", base))
    story.append(bullet("<b>Сен 2022 - окт 2022 · Помощник организатора, АРСК.</b> Помощь в организации мероприятий, согласование разрешений и поддержки проекта, ведение документооборота и взаимодействие с командами. <link href='https://druzhinskaia.github.io/recommendations/recommendation-arsk-2022.pdf'>Рекомендательное письмо</link>.", base))

    story.append(p("Портфолио проектов", h2))
    story.append(p("Материалы основаны на коммерческих задачах; открытые данные и отдельные показатели обезличены и реконструированы для соблюдения конфиденциальности.", small))
    projects = [
    ...
    story.append(
        p(
            "Сертификат: «Кадровый учет в УАИС „Бюджетный учет“. 1С», 2026. "
            "Роль в программе — кадровик; срок действия сертификата — 3 года.",
            base,
        )
    )
