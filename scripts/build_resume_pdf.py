...
def build() -> None:
    ...
    story.append(header)
    story.append(p("Аналитик отчетности, данных и процессов", role))
    story.append(
        p(
            "1C · Excel · SQL · Python · BPMN · Power BI (базовый) · контроль качества данных",
            small,
        )
    )
    story.append(
        p(
            "Более четырех лет работаю с операционными данными, отчетностью и процессами: веду и "
            "проверяю реестры и выгрузки, автоматизирую отчетность и готовлю понятные артефакты для бизнеса.",
            base,
        )
    )
    ...
    three_cols = Table(
        [
            [
                ...
                p("<b>Инструменты</b><br/>Excel, 1C-выгрузки, SQL, Python/pandas, Power BI (базовый), BI-прототипы, документация.", cell),
