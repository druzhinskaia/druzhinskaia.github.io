...
function drawioEdge(id, source, target, label = "") {
  return `    <mxCell id="${id}" value="${esc(label)}" style="edgeStyle=orthogonalEdgeStyle;rounded=0;orthogonalLoop=1;jettySize=auto;html=1;strokeColor=${COLORS.navy};strokeWidth=2;endArrow=block;endFill=1;" edge="1" parent="1" source="${source}" target="${target}">
...
function drawioDiagram(name, width, height, lanes, nodes, edges) {
  ...
  const edgeCells = edges.map((edge, idx) => drawioEdge(`e${idx + 1}`, edge.from, edge.to, edge.label || ""));
...
const lanes = [
  { id: "lane-client", label: "Инициатор", x: 20, y: 80, width: 1480, height: 120 },
  { id: "lane-manager", label: "Менеджер", x: 20, y: 200, width: 1480, height: 170 },
  { id: "lane-1c", label: "1С", x: 20, y: 370, width: 1480, height: 140 },
  { id: "lane-head", label: "Руководитель", x: 20, y: 510, width: 1480, height: 150 },
  { id: "lane-exec", label: "Исполнитель", x: 20, y: 660, width: 1480, height: 120 },
];
...
const asIsNodes = [
  ...
  { id: "as-approve", label: "Согласование вне системы", style: manualStyle, x: 1090, y: 555, width: 170, height: 70 },
  { id: "as-gateway-approve", label: "Согласовано?", style: gatewayStyle, x: 1300, y: 545, width: 110, height: 90 },
  { id: "as-rework", label: "Вернуть на доработку", style: manualStyle, x: 1080, y: 250, width: 170, height: 70 },
  { id: "as-execute", label: "Передать исполнителю", style: manualStyle, x: 1270, y: 700, width: 170, height: 60 },
  { id: "as-close", label: "Закрыть заявку и статус вручную", style: manualStyle, x: 1050, y: 700, width: 170, height: 60 },
  { id: "as-end", label: "Заявка закрыта", style: endStyle, x: 850, y: 700, width: 120, height: 60 },
  { id: "as-note-1", label: "Проблема: нет единого канала и обязательных полей", style: noteStyle, x: 250, y: 95, width: 210, height: 80 },
  { id: "as-note-2", label: "Проблема: статусы и сроки контролируются вручную", style: noteStyle, x: 970, y: 385, width: 230, height: 80 },
];

const asIsEdges = [
  { from: "as-start", to: "as-receive" },
  { from: "as-receive", to: "as-check" },
  { from: "as-check", to: "as-gateway-data" },
  { from: "as-gateway-data", to: "as-clarify", label: "Нет" },
  { from: "as-clarify", to: "as-check" },
  { from: "as-gateway-data", to: "as-create-1c", label: "Да" },
  { from: "as-create-1c", to: "as-approve" },
  { from: "as-approve", to: "as-gateway-approve" },
  { from: "as-gateway-approve", to: "as-rework", label: "Нет" },
  { from: "as-rework", to: "as-check" },
  { from: "as-gateway-approve", to: "as-execute", label: "Да" },
  { from: "as-execute", to: "as-close" },
  { from: "as-close", to: "as-end" },
];
...
const toBeNodes = [
  ...
  { id: "to-work", label: "Менеджер принимает в работу", style: taskStyle, x: 1160, y: 250, width: 180, height: 70 },
  { id: "to-gateway-approve", label: "Нужно согласование?", style: gatewayStyle, x: 1160, y: 545, width: 130, height: 90 },
  { id: "to-approve", label: "Руководитель согласует в 1С", style: taskStyle, x: 1350, y: 555, width: 170, height: 70 },
  { id: "to-execute", label: "Исполнитель выполняет заявку", style: taskStyle, x: 1320, y: 700, width: 180, height: 60 },
  { id: "to-close", label: "1С фиксирует закрытие и обновляет отчет", style: systemStyle, x: 1060, y: 700, width: 210, height: 60 },
  { id: "to-end", label: "Заявка закрыта", style: endStyle, x: 850, y: 700, width: 120, height: 60 },
...
const toBeEdges = [
  { from: "to-start", to: "to-validate" },
  { from: "to-validate", to: "to-gateway-valid" },
  { from: "to-gateway-valid", to: "to-return", label: "Нет" },
  { from: "to-return", to: "to-start" },
  { from: "to-gateway-valid", to: "to-create", label: "Да" },
  { from: "to-create", to: "to-assign" },
  { from: "to-assign", to: "to-work" },
  { from: "to-assign", to: "to-notify" },
  { from: "to-assign", to: "to-report" },
  { from: "to-work", to: "to-gateway-approve" },
  { from: "to-gateway-approve", to: "to-approve", label: "Да" },
  { from: "to-gateway-approve", to: "to-execute", label: "Нет" },
  { from: "to-approve", to: "to-execute" },
  { from: "to-execute", to: "to-close" },
  { from: "to-close", to: "to-end" },
];
...
function svgEdge(edge, nodes) {
  ...
  const midX = (x1 + x2) / 2;
  const label = edge.label ? `<text x="${midX}" y="${(y1 + y2) / 2 - 6}" text-anchor="middle" font-size="12" fill="${COLORS.navy}">${esc(edge.label)}</text>` : "";
  return `<path d="M ${x1} ${y1} L ${midX} ${y1} L ${midX} ${y2} L ${x2} ${y2}" fill="none" stroke="${COLORS.navy}" stroke-width="2" marker-end="url(#arrow)"/>${label}`;
}
...
function svgDiagram(title, nodes, edges) {
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" width="1520" height="820" viewBox="0 0 1520 820">
  ...
  <text x="760" y="45" text-anchor="middle" font-size="24" font-weight="700" fill="${COLORS.navy}">${esc(title)}</text>
  ${laneSvg}
...
async function main() {
  ...
  await fs.writeFile(path.join(diagramsDir, "as-is-process.drawio"), drawioDiagram("AS-IS process", 1520, 820, lanes, asIsNodes, asIsEdges));
  await fs.writeFile(path.join(diagramsDir, "to-be-process.drawio"), drawioDiagram("TO-BE process", 1520, 820, lanes, toBeNodes, toBeEdges));
